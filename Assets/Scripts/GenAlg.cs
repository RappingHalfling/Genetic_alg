...
public class GenAlg : MonoBehaviour
{
    ...
    List<CarBrain> CurCarGeneration = new List<CarBrain>();
    ...
    private void FillFirstGeneration()
    {
        for (int i = 0; i < SamplesPerGeneration; i++)
        {
            CarBrain car = transform.GetChild(i).GetComponent<CarBrain>();
            NNClass NN = car.GetComponent<NNClass>();
            CurNNGeneration.Add(NN);
            CurCarGeneration.Add(car);
            Parents.Add(null);
    ...
    private void ChooseParentsByRoulette()
    {
        ...
        for (int i = 0; i < SamplesPerGeneration; i++)
        {
            ...
            for (int j = 0; j < SamplesPerGeneration; j++)
            {
                ...
                {
                    tempNN = CurCarGeneration[i].gameObject.AddComponent<NNClass>();
                    tempNN.InputsNum = CurNNGeneration[j].InputsNum;
                    tempNN.OutsNum = CurNNGeneration[j].OutsNum;
                    tempNN.Initialization(CurCarGeneration[i].LayersNum, CurCarGeneration[i].NeuronPerLayer);
    ...
    // ВЫборка по ранг(Проходит только первая половина особей со своии дубликатами)
    private void ChooseParentsByRank()
    {
        ...
        {
            tempNN = CurCarGeneration[SamplesPerGeneration - 1 - i].gameObject.AddComponent<NNClass>();
            tempNN1 = CurCarGeneration[i].gameObject.AddComponent<NNClass>();
            ...
            tempNN.Initialization(CurCarGeneration[i].LayersNum, CurCarGeneration[i].NeuronPerLayer);
            tempNN1.Initialization(CurCarGeneration[i].LayersNum, CurCarGeneration[i].NeuronPerLayer);
            for (int k = 0; k < tempNN.weights.Count; k++)
            {
                for (int x = 0; x < tempNN.weights[k].RowCount; x++)
                {
    ...
    {
        int LayersNum = CurCarGeneration[0].LayersNum;
        int NeuronPerLayer = CurCarGeneration[0].NeuronPerLayer;
        ...
        {
            if(j != NumForElitist)
            DestroyImmediate(CurCarGeneration[j].gameObject.GetComponent<NNClass>());  
            else
            DestroyImmediate(CurCarGeneration[j].gameObject.GetComponents<NNClass>()[1]);
        }       
    }
    ...
    private void ResetGen()
    {
        ...
        for (int i = 0; i < SamplesPerGeneration; i++)
        {
            ...
            CurNNGeneration[i] = CurCarGeneration[i].gameObject.GetComponent<NNClass>();
            CurCarGeneration[i].ResetCar(CurNNGeneration[i]);
...
;
    ...
    // Проверка Окаменелости всех осыбей
    private bool ChekcAllStone()
    {
        bool check = true;
        for (int i = 0; i < SamplesPerGeneration; i++)
        {
            if (CurNNGeneration[i].Fitness >= 2000)
            {
                check = true;
                break;
            }
            check = check && CurCarGeneration[i].IamStone;
        }
        return check;
    }
